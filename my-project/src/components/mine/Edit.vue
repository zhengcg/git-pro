<template>
  <div id="edit">
    ...
    <a href="javascript:;" class="saveBtn" @click="updateData">保存</a>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    getAddress(id){
      var self=this;
      Indicator.open();
        this.axios.get(api.getAddId,{params: {uuid:id}}).then(function (res) {
            Indicator.close();
            if(res.data.code==200){             
              var data=res.data.result
              self.formData.uuid=data.uuid;
              self.provinceValue={id:data.provinceId,name:data.province}
              self.getCity();             
              self.cityValue={id:data.cityId,name:data.city}
              self.getArea();
              self.countyValue={id:data.countyId,name:data.county}
              self.formData.detailAddress=data.detailAddress;
              self.formData.postCode=data.postCode;
              self.formData.receiver=data.receiver;
              self.formData.receiverPhone=data.receiverPhone;
    ...
    updateData:function(){
      ...
      if(self.validate()){
        ...
        this.axios.post(api.updateAddress, self.formData)
          .then(function (res) {
            Indicator.close();
            if(res.data.code==200){
              self.$router.push({
